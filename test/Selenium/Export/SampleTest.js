...
var browser, element, currentCommand = '',
    options = {
        timeout: 30000,
        retries: 0,
        screenshotFolder: 'screenshots/SampleTest',
        baseUrl: 'http://change-this-to-the-site-you-are-testing/'
    };

module.exports = function testSample(_browser, _options) {

    browser = _browser;
    var acceptNextAlert = true;
    getRuntimeOptions(_options);
    try {
        currentCommand = 'open("http://localhost:3999", "")';
        browser.get(addBaseUrl("http://localhost:3999"));

        currentCommand = 'assertText("css=h2", "The den is dark")';
        var var1 = "The den is dark";
        var var2 = browser.elementByCssSelector("h2").text();
        assert.equal(var1, var2, 'Assertion error: Expected: ' + var1 + ', got: ' + var2);

        currentCommand = 'click("css=button", "")';
        browser.elementByCssSelector("button").click();

        currentCommand = 'assertText("css=h2", "DRAGON HAS ENTERED")';
        var var3 = "DRAGON HAS ENTERED";
        var var4 = browser.elementByCssSelector("h2").text();
        assert.equal(var3, var4, 'Assertion error: Expected: ' + var3 + ', got: ' + var4);
